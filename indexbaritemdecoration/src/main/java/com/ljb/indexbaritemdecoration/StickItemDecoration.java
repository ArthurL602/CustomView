...
public class StickItemDecoration<T extends TitleBean> extends RecyclerView.ItemDecoration {
    ...
    public StickItemDecoration(Context context, List<T> datas) {
        super();
        mDatas = datas;
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        //判断当前item与下一个item的标签是否一致
        if (pos + 1 < mDatas.size() && !mDatas.get(pos).getTag().equals(mDatas.get(pos + 1)
                .getTag())) {

            View child = parent.findViewHolderForLayoutPosition(pos).itemView;
            //从这里开始检测是否碰撞，可以选择控制canvas，也可以直接该top值
            if (child.getTop() + child.getMeasuredHeight() < mTitleHeight) {
//                c.save();
//                flag = true;
//                c.translate(0, child.getTop() + child.getMeasuredHeight() - mTitleHeight+1);//负的代表向上
                top=child.getTop() + child.getMeasuredHeight() - mTitleHeight+1;
            }
        ...
        // 换成 parent.findViewHolderForLayoutPosition(pos).itemView
        View child = parent.findViewHolderForLayoutPosition(pos).itemView;
        mPaint.setColor(Color.GRAY);
        //绘制标题背景区域
        c.drawRect(left, top, right, bottom, mPaint);
        ...
        c.drawText(tag, child.getPaddingLeft(), top + (mTitleHeight / 2 + rect.height() / 2), mPaint);
//        if (flag) {
//            //还原画布为初始状态
//            c.restore();
//        }
